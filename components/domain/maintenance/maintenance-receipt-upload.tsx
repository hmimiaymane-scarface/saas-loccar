...
import { attachMaintenanceReceipt } from "@/app/(dashboard)/maintenance/actions"
import { cn } from "@/lib/utils"

function MaintenanceReceiptUpload({
  maintenanceId,
  companyId,
  hasReceipt,
}: {
  ...
  async function handleFile(file: File) {
    ...
    setBusy(true)
    const path = buildStoragePath(companyId, ["maintenance", maintenanceId], file.name)
    ...
    const result = await attachMaintenanceReceipt(maintenanceId, path)
    setBusy(false)
    if (result.error) {
...
export { MaintenanceReceiptUpload }
